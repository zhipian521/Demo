...
const ResolveQuestionModal: FC<ResolveQuestionModalProps> = (props) => {
    ...
    >
      <Form {...formLayout} form={form} onFinish={handleFinish}>
        <Form.Item label="规则" name="ruleCode" rules={[{ required: true, message: '请选择规则' }]}>
          <Select placeholder="支持编码和名称搜索" showSearch disabled>
            {ruleEnums
              ? ruleEnums.map((item) => (
        ...
        >
          <Input.TextArea disabled autoSize={{ minRows: 5 }} placeholder="请输入问题描述" />
        </Form.Item>
        ...
        >
          <Input.TextArea disabled autoSize={{ minRows: 5 }} placeholder="请输入解决方案" />
        </Form.Item>
        <Form.Item
          name="solution"
          label="解决方案"
...
export default ResolveQuestionModal;
